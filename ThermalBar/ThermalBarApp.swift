...
func createMenuBarImage(
  items: [String], isVertical: Bool, iconName: String = "thermometer.medium", showIcon: Bool = true
) -> NSImage {
  ...
  if showIcon {
    let iconRect = NSRect(
      x: 0, y: (totalHeight - iconHeight) / 2, width: iconWidth, height: iconHeight)
  ...
  // System tint adapts this to Light/Dark mode automatically
...
func createLinearProgressMenuBarImage(label: String, value: Double) -> NSImage {
  ...
  let barY = (totalHeight - barHeight) / 2.0
  ...
  let trackColor =
    isDarkMode ? NSColor.white.withAlphaComponent(0.2) : NSColor.black.withAlphaComponent(0.2)
  ...
  let bgPath = NSBezierPath(
    roundedRect: NSRect(x: barX, y: barY, width: barWidth, height: barHeight), xRadius: 2.5,
    yRadius: 2.5)
  trackColor.set()
  bgPath.fill()
  ...
  if value > 0.0 {
    ...
    let fillPath = NSBezierPath(
      roundedRect: NSRect(x: barX, y: barY, width: fillWidth, height: barHeight), xRadius: 2.5,
      yRadius: 2.5)
    fillColor.set()
    fillPath.fill()
  }
...
func createCombinedVerticalUsageMenuBarImage(cpu: Double?, ram: Double?, gpu: Double?) -> NSImage {
  ...
  if activeMetrics.isEmpty {
    // Return a minimal 1×1 transparent image to keep the menu bar slot alive
    let img = NSImage(size: NSSize(width: 1, height: totalHeight))
    return img
  }
  ...
  for metric in activeMetrics {
    let labelSize = NSAttributedString(string: metric.label, attributes: tempAttributes).size()
      .width
    let pctSize = NSAttributedString(
      string: "\(Int(metric.val.rounded()))%", attributes: tempAttributes
    ).size().width
  ...
  for (index, metric) in activeMetrics.enumerated() {
    ...
    // Center the bar vertically with the text on this row
    let barY = yOffset + (fontSize - barHeight) / 2.0
    ...
    let bgPath = NSBezierPath(
      roundedRect: NSRect(x: barX, y: barY, width: barWidth, height: barHeight), xRadius: radius,
      yRadius: radius)
    trackColor.set()
    bgPath.fill()
    ...
    if metric.val > 0.0 {
      ...
      let fillPath = NSBezierPath(
        roundedRect: NSRect(x: barX, y: barY, width: fillWidth, height: barHeight), xRadius: radius,
        yRadius: radius)
...
private final class MenuBarImageCache {
  ...
  private var tempCacheOrder: [String] = []

  private var combinedUsageImageCache: [String: NSImage] = [:]
  private var combinedCacheOrder: [String] = []

  private var linearUsageImageCache: [String: NSImage] = [:]
  private var linearCacheOrder: [String] = []
  ...
  private init() {
    updateIsDarkMode()
    // Observe effectiveAppearance changes to keep isDarkMode in sync
  ...
  private func appearanceChanged() {
    updateIsDarkMode()
    // Flush caches on theme switch so old theme‑specific images are released
    tempImageCache.removeAll(keepingCapacity: false)
    tempCacheOrder.removeAll()
    combinedUsageImageCache.removeAll(keepingCapacity: false)
    combinedCacheOrder.removeAll()
    linearUsageImageCache.removeAll(keepingCapacity: false)
    linearCacheOrder.removeAll()
  }
  ...
  private func evictIfNeeded(cache: inout [String: NSImage], order: inout [String]) {
    while cache.count > Self.maxCacheSize, let oldest = order.first {
  ...
  func getTempImage(items: [String], isVertical: Bool, showIcon: Bool) -> NSImage {
    ...
    tempCacheOrder.append(key)
    evictIfNeeded(cache: &tempImageCache, order: &tempCacheOrder)
    return newImg
  }
  ...
  func getCombinedUsageImage(cpu: Double?, ram: Double?, gpu: Double?) -> NSImage {
    ...
    combinedCacheOrder.append(key)
    evictIfNeeded(cache: &combinedUsageImageCache, order: &combinedCacheOrder)
    return newImg
  }
  ...
  func getLinearUsageImage(label: String, value: Double) -> NSImage {
    ...
    linearCacheOrder.append(key)
    evictIfNeeded(cache: &linearUsageImageCache, order: &linearCacheOrder)
    return newImg
  }
}
...
struct ThermalBarApp: App {
  ...
  var body: some Scene {
    MenuBarExtra {
      ...
      Image(
        nsImage: MenuBarImageCache.shared.getTempImage(
          items: items, isVertical: viewModel.menuBarTextOrder == "Vertical",
          showIcon: viewModel.showMenuBarIcon)
      )
      .id(
        "\(items.joined(separator: "-"))-\(viewModel.menuBarTextOrder)-\(viewModel.showMenuBarIcon)"
      )
    }
    ...
    MenuBarExtra(
      isInserted: Binding(
        get: {
          (viewModel.showCpuMenuBar || viewModel.showRamMenuBar || viewModel.showGpuMenuBar)
            && viewModel.systemUsageLayout == "Vertical"
        },
        set: { _ in }
      )
    ) {
      ...
      let cacheId =
        "combined-vertical-\(Int(cpuVal ?? -1))-\(Int(ramVal ?? -1))-\(Int(gpuVal ?? -1))"
      Image(
        nsImage: MenuBarImageCache.shared.getCombinedUsageImage(
          cpu: cpuVal, ram: ramVal, gpu: gpuVal)
      )
      .id(cacheId)
    }
    .menuBarExtraStyle(.window)
